...
class PGAgent(nn.Module):
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            # if no baseline, then q_values are advantage
            advantages = q_values
        else:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    delta_t = rewards[i] - values[i] + (1 - terminals[i]) * values[i+1] * self.gamma
                    advantages[i] = delta_t + (1 - terminals[i]) * self.gae_lambda * self.gamma * advantages[i + 1]
        ...
        if self.normalize_advantages:
            ...
            std_deviation = advantages.std() 
            eps = 1e-8
            advantages = (advantages - mean) / (std_deviation + eps)
        return advantages
