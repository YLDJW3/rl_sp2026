...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = nn.Parameter(torch.log(torch.tensor(self.temperature)))
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_action = next_action_distribution.sample() # (N, ac_dim)
            next_qs = self.target_critic(next_obs, next_action)    # (critic, N)
            # ENDTODO

            if self.use_entropy_bonus and self.backup_entropy:
                # TODO(Section 3.3): Add entropy bonus to the target values for SAC
                next_action_entropy = -next_action_distribution.log_prob(next_action)   # (N, )
                assert next_action_entropy.shape == (
                    batch_size,
            ...
            next_qs = self.q_backup_strategy(next_qs)   # (critic, N)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        action = action_distribution.sample()   # (N, ac_dim)
        entropy = -action_distribution.log_prob(action)  # (N, )
        assert entropy.shape == (action.shape[0], )
        return entropy
        # ENDTODO
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        loss = -self.q_backup_strategy(q_values).mean()    # (critic, N, )
        # ENDTODO
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        entropy_bonus = -log_prob.mean()  # (N, )
        loss -= self.get_temperature() * entropy_bonus
        # ENDTODO
    ...
    def update_alpha(self, log_prob: torch.Tensor):
        ...
        alpha_loss = -alpha * (log_prob.mean() + self.target_entropy)
    ...
    def update(
        ...
        if self.target_update_period:
            ...
            raise ValueError("invalid target update approach")
        # ENDTODO
